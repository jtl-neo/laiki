...
/**
 * Load the user's context from the database.
 * Stub until the shadow-account / payment-method tables land (Batch 3);
 * the interface is frozen now so parseUnified call sites don't churn.
 */
export async function loadParseContext(_userId: string): Promise<ParseContext> {
  return emptyContext();
}
